...
BEGIN
  ...
  -- Rank creators by activity (plays + upvotes across their public setlists,
  -- tiebreak by number of public setlists, then earliest joined).
  -- For each candidate setlist, prefer creators ranked highest who have
  -- gone the longest without a feature. Deterministic — no random.
  ...
  setlist_last_feature AS (
    SELECT setlist_id, MAX(spotlight_date) AS last_featured
    FROM public.hero_spotlights
    GROUP BY setlist_id
  )
  ...
  LEFT JOIN setlist_last_feature slf ON slf.setlist_id = sl.id
  WHERE sl.is_public = true
    AND EXISTS (SELECT 1 FROM public.setlist_slots ss WHERE ss.setlist_id = sl.id)
  ORDER BY
    -- Round-robin across creators: whoever was featured longest ago goes next
    clf.creator_last_featured ASC NULLS FIRST,
    rc.creator_rank ASC,
    -- Within that creator, prefer their least-recently-featured setlist

-- Hero spotlight: require at least 3 playable songs (not just 1) so the
-- "Now Spinning" CTA always surfaces a setlist that plays as a real session.
-- Also drop today's weak pick (only 1 playable song) so the next call rotates.

DELETE FROM public.hero_spotlights
WHERE setlist_id IN (
  SELECT s.id
  FROM public.setlists s
  LEFT JOIN public.setlist_slots ss ON ss.setlist_id = s.id
  LEFT JOIN public.notable_versions nv ON nv.id = ss.notable_version_id AND nv.archive_org_url IS NOT NULL
  GROUP BY s.id
  HAVING count(nv.id) < 3
);

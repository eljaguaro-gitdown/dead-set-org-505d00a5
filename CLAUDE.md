...
- **Static verification silently conflates "defined" with "wired."** Code that looks instrumented can be dead or depend on infrastructure invisible to a repo-only sweep. When a checklist item names a mechanism, grep for **callers**, not definitions, and report "defined but never called" as its own finding — distinct from "unobservable because no browser." The two examples banked here have since diverged, and a stale correction misleads the next gate, so both are dated:
  - **PostHog — HALF FIXED, and the unfixed half is still dropping events (verified 2026-09-19 at `3bcde32`).** The original note said every `window.posthog?.capture(...)` call site was optional-chained and nothing ever `.init()`ed the SDK. The second half stopped being true on 2026-09-18: `a4060b8` ("Instrument PostHog via the official wizard") and `0de4124` adopted the `posthog-js` package, `src/main.tsx:7` now calls `initializePostHog()` unconditionally, and [`src/lib/posthog.ts`](src/lib/posthog.ts) wraps the SDK as `captureEvent` / `captureException` / `identifyUser` behind an `isPostHogConfigured` token-and-host guard. **An event fired through `captureEvent` is genuinely wired.** But the old global pattern survived in one file and is now a silent no-op: [`src/lib/player/audioInstrumentation.ts`](src/lib/player/audioInstrumentation.ts) still reads `(window as Window & PostHogGlobal).posthog` and calls `ph?.capture?.(...)`, wrapped in a swallowing `try/catch`. `posthog-js` **does not** set `window.posthog` when imported as a module — its only global-looking assignment is a CommonJS `exports.posthog`, nothing under `dist/` contains the literal `window.posthog`, and `index.html` carries no snippet — so `ph` is `undefined` and all five audio events (`audio_play_started`, `audio_track_advanced`, `audio_segue_completed`, `audio_error`, `audio_autoplay_blocked`) are dropped at their 6 call sites, taking the gapless-transition wild-signal with them. The fix is to route `captureAudioEvent` through `captureEvent` like every other surface. **Two PostHog patterns coexist here — tell them apart per call site rather than assuming the file is uniform**, and check which one a new event uses before calling it wired OR calling it dead.
  - **`draft_setlists` — STILL TRUE, re-verified at `3bcde32` (2026-09-19).** The RLS anonymous-draft architecture still has no caller outside its own test: `draft_setlists` appears in `src/` only in generated `types.ts` and in two comments inside [`src/lib/anonSession.ts`](src/lib/anonSession.ts), which itself has no importers. The shipping mechanism remains session-scoped `sessionStorage`.

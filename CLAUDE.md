...
- **Release state lives in App Store Connect, not in this repo.** The repo showed no workflow run history, so a session concluded the TestFlight pipeline "had never run green" and reported it as a submission risk — twice, in writing. App Store Connect had **19 completed build uploads**, all Ready to Submit. Builds 13–19 went up from Xcode, which leaves no trace here. Before making any claim about builds, pipeline health, or what has been submitted, look at App Store Connect. Absence of evidence in the repo is not evidence about a system whose state lives elsewhere.
- **The CI build number needs its offset.** `ios-testflight.yml` sets `CURRENT_PROJECT_VERSION` from `github.run_number + BUILD_NUMBER_OFFSET`. The offset is not decoration: builds 1–19 were uploaded before that workflow existed, so an unoffset first run would archive and sign correctly and then die at upload on a duplicate build number — a failure that reads exactly like a signing or export problem. Raise the offset if builds are ever uploaded by another route again.
